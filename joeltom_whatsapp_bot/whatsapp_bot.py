#!/usr/bin/env python3
"""
𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 — Bot WhatsApp v1 (Twilio + Flask)
Gestion SSH/XRAY/ZIVPN + Gestion revendeurs via WhatsApp
Contact WhatsApp: wa.me/237654145540
groupe: https://chat.whatsapp.com/Em6sqV7E5CfChcIQk2jsUe?s=cl&p=a&mlu=4
"""
...
logger = logging.getLogger('joeltom-whatsapp')

CONFIG_PATH = os.environ.get('𝑱𝑶𝑬𝑳𝑻𝑶𝑴_WA_CONFIG', '/etc/joeltom_whatsapp_bot/config.json')
DEFAULT_CFG = {'twilio_account_sid': '', 'twilio_auth_token': '', 'twilio_whatsapp_number': '', 'admin_whatsapp_number': '', 'bot_password': 'joeltom2026'}
...
BOT_PASSWORD  = cfg.get('bot_password', 'joeltom2026')

if not TWILIO_SID or not TWILIO_TOKEN:
    raise SystemExit('Missing Twilio credentials. Set 𝑱𝑶𝑬𝑳𝑻𝑶𝑴_WA_CONFIG or /etc/joeltom_whatsapp_bot/config.json')
...
HELP_ADMIN = """*𝑱𝑶𝑬𝑳𝑻𝑶𝑴 — Commandes Admin*
...
HELP_RESELLER = """*𝑱𝑶𝑬𝑳𝑻𝑶𝑴 — Commandes Revendeur*
...
def process_command(phone: str, text: str) -> str:
    ...
    if not is_auth:
        if text == BOT_PASSWORD or text == f"connexion {BOT_PASSWORD}":
            ...
            return f"🔐 *𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 Bot*\n\nEntrez votre mot de passe pour continuer.\n\n_Contact: wa.me/237654145540_"
...
def whatsapp_webhook():
    ...
    try:
        ...
        resp.message("❌ Erreur interne. Réessayez ou contactez @joeltom23")
    return str(resp)

@app.route('/health')
def health():
    return {'status': 'ok', 'service': 'joeltom-whatsapp-bot'}

if __name__ == '__main__':
    port = int(cfg.get('port', 5001))
    logger.info(f"𝑱𝑶𝑬𝑳𝑻𝑶𝑴 WhatsApp Bot v2 starting on port {port}...")
