#!/usr/bin/env python3
"""
𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 — Bot Deploy Multi-VPS v1
Déploie 𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 sur plusieurs VPS en parallèle via SSH Paramiko
Contact: @joeltom3
"""
...
CONFIG_PATH = os.environ.get('𝑱𝑶𝑬𝑳𝑻𝑶𝑴_DEPLOY_CONFIG', '/etc/joeltom_deploy_bot/config.json')
...
INSTALL_URL = cfg.get('install_url', 'https://raw.githubusercontent.com/joeltom-tech/Script_joeltom/main/autoinstall.sh')

if not BOT_TOKEN:
    raise SystemExit('Missing Telegram bot token. Set 𝑱𝑶𝑬𝑳𝑻𝑶𝑴_DEPLOY_CONFIG or /etc/katashie_deploy_bot/config.json')
...
BANNER = "💻 <b>𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 Deploy Bot</b>\n📲 @joeltom23"
...
def deploy_to_vps(host, port, user, password, ssh_key, vps_name, chat_id, msg_id):
    """Deploy 𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 on a single VPS with live streaming"""
    try:
        ...
        cmd = f"wget -qO /tmp/joeltom_install.sh '{INSTALL_URL}' && bash /tmp/katashie_install.sh 2>&1"
...
if __name__ == '__main__':
    logger.info("𝑱𝑶𝑬𝑳_𝑻𝑶𝑴 Deploy Bot v1 starting...")
